...
def tessellate(int[::1] cables_idx, int[::1] cable_len, int cable_sep, 
    ...
    Args:
        cables_idx (int32 array): concatenation of arrays of vertex indices, each 
        ...
        sign (uint8 array): generated by create_cable_network; it relates to the 
            surface orientation
    ...
    Returns:
        facets (list of tuples or list of arrays): each facet is represented by 
            ...
            4 times to the left to create a square
        vneigh (nv-by-4 int array): if return_neigh is True, returns the indices 
            of up to 4 neighboring vertices of each vertex; 
...
def facet_size(list facets):
    ...
    Args:
        facets (list of int arrays or list of tuples): generated by 'tessellate'
...
def group_facets_by_size(list facets, int cutoff=1000000):
    ...
    Returns:
        ...
        mapping (int array): for each output facet, gives the index of the facet 
...
def find_neighbors(int[::1] cables_idx, int[::1] cable_len, int cable_sep, 
                   char[::1] sign):
    """Deterning the nearest neigbors of vertices, create the set of the edges 
    ...
    Returns:
        ...
        heneigh (2*ne-by-3 int array): indices of up to 3 neighboring half-edges 
            of each half-edge; the three columns correspond to half-edges in the
            left, forward and right directions respectively (relative to the 
            orientation of the triangular mesh); index is -1 if there is no 
            neighbor
    """
    cdef:
...
def identify_facets_and_directions(int[:, ::1] halfedges, int[:, ::1] heneigh):
    ...
    Returns:
        facets (list of tuples): each facet is represented by an array of vertex 
            indices and by a string indicating of the moves are in the left (L), 
            right (R) or forward (F) direction, e.g. 'LLLL' means the path 
            requires to go 4 times to the left to create a square
    """
    cdef:
        list loop, facets = []
        str moves
